...
class CreateZipDialog(wx.Dialog):
    ...
    def on_ok(self, event):
        ...
        if not (path or self.auto_naming_box.GetValue()):
            wx.MessageDialog(self, "Devi specificare entrambi i campi!", ERROR_CAPTION, style=ERROR_STYLE).ShowModal()
            return
        if not os.path.exists(path):
            wx.MessageDialog(self, "La cartella specificata non esiste.", ERROR_CAPTION, style=ERROR_STYLE).ShowModal()
            return
        if os.path.splitext(name)[1].lower() != '.zip':
            name += '.zip'
            self.name_text.SetValue(name)
        if name in os.listdir(path):
            wx.MessageDialog(self, "%s esiste gi�. Specifica un altro nome." %name, ERROR_CAPTION, style=ERROR_STYLE).ShowModal()
            return
