    ...
    def OnDropFiles(self, x, y, filenames):
        ...
        for file in filenames:
            if os.path.isdir(file):
                list.extend([os.path.join(file, p) for p in os.listdir(file)])
            else:
                list.append(file)
        list = [p for p in list if os.path.isfile(p) and imghdr.what(p)]
...
class ImageWindow(wx.ScrolledWindow):
    ...
    def show_goto_img(self, event):
        ...
        self.parent.n = (
            self.from_goto - 1 - 1
        )  # -1 because of 0-based index, -1 to step on the previous image
    ...
    def mark_all_to_be_saved(self, event):
        ...
        dial = wx.MessageDialog(
            None,
            "Do you want to mark all images to be saved?",
            "Question",
    ...
    def zoom(self, percent, quality="BILINEAR"):
        ...
        if self.image:
            ...
            self.image = pil_to_image(
                bmp_to_pil(self.image).resize((nw, nh), getattr(Image, quality))
            )
            ...
            wx.MessageDialog(
                self,
                "Seleziona un'immagine prima",
                resources.ERROR_CAPTION,
                style=resources.ERROR_STYLE,
            ).ShowModal()
