...
class MReader(wx.Frame):
    def __init__(self, title, *args):
        ...
        # TOOLBAR
        self.toolbar = resources.MyToolBar(
            self, -1, style=wx.TB_TEXT | wx.TB_HORIZONTAL | wx.TB_FLAT
        )
        self.toolbar.AddMany(
            [
                (ID_OPEN_DIR, "Open local", wx.Bitmap(imgpath("document-open.png"))),
                # "sep",
    ...
    def show_about(self, event):
        # print sys._getframe().f_code.co_name
        about_dialog = resources.HtmlMessageDialog(
            self, "HenWer | About", "html/about.html", size=(350, 320)
        )
        about_dialog.Center()
    ...
    def show_key_bindings(self, event):
        # print sys._getframe().f_code.co_name
        key_dialog = resources.HtmlMessageDialog(
            self, "HenWer | Key Bindings", "html/key_bindings.html", size=(400, 600)
        )
        key_dialog.Center()
    ...
    def confirm_commit(self, event):
        ...
        if commit.there_is_something_to_commit(self):
            ...
            dial = wx.MessageDialog(
                None, msg, "Question", wx.YES_NO | wx.YES_DEFAULT | wx.ICON_QUESTION
            )
    ...
    def open_dir(self, event, dir_to_open=None):
        ...
        if self.local_file == 1:
            flist = [
                os.path.join(path, el)
                for el in os.listdir(path)
                if os.path.isfile(os.path.join(path, el))
                and imghdr.what(os.path.join(path, el)) is not None
            ]
    ...
    def download_to_local(self, img):
        if self.referer is None:
            urllib.request.urlretrieve(img.path, img.get_tmp_file_to_save_to())
        else:
    ...
    def check_save_dir(self, save_dir):
        ...
        if save_dir is None:
            msg = (
                "No save_dir is specified in your preferences, thus "
                + "you won't be able to save images."
            )
            problem = True
        elif os.path.exists(save_dir) == False:
            msg = (
                "The save_dir directory specified in your preferences doesn't exist, thus "
                + "you won't be able to save images."
            )
            problem = True
        elif os.path.isdir(save_dir) == False:
            msg = (
                "The save_dir directory specified in your preferences is not a directory, thus "
                + "you won't be able to save images."
            )
            problem = True
        if problem:
